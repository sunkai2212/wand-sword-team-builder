...
**Goal:** 交付一个公开、轻量、手机优先的静态网页，用户可编排 5×4 站位、配置四名角色的技能与宠物，并导出 PNG 长图。
...
`src/data/skills.json` 为每个可选技能保存一条记录；名称必须与截图一致，不记录等级、品质和数值。七转每个职业应有 12 条技能记录。宠物清单必须精确为：
...
- [ ] **Step 1: 写未完整配置不能出图的失败测试**

```ts
test("keeps export disabled until all four members are complete", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "七转" }).click();
  await expect(page.getByRole("button", { name: "生成图片" })).toBeDisabled();
  await expect(page.getByText("还需要添加4名角色")).toBeVisible();
});
```
...
页面底部固定显示完成度。缺成员时显示“还需要添加 N 名角色”；缺技能或宠物时高亮对应角色卡和空槽。只有 `completionIssues(team).length === 0` 时启用“生成图片”。
...
4. 完整四人配置可导出 1080×1920 PNG，站位、技能顺序与宠物全部正确。
